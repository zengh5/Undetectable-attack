...
for k in range(0, num_batches):
    ...
    for t in range(max_iterations):
        ...
        # loss = nn.CrossEntropyLoss(reduction='sum')(logits, labels)    # targeted
        loss = nn.CrossEntropyLoss(reduction='sum')(logits, labels_ori)  # un-targeted
        loss.backward()
        grad_c = delta.grad.clone()

        delta.grad.zero_()
        # delta.data = delta.data - lr * torch.sign(grad_c)              # targeted
        ...
        if t % 50 == 49:
            ...
            for img_i in range(batch_size_cur):
                ...
                save_path = 'advimgs_un/ifgsm/' + image_id_list[k * batch_size + img_i] + '.png'
...
for k in range(0, num_batches):
    ...
    for t in range(max_iterations):
        ...
        if t % 50 == 49:
            ...
            for img_i in range(batch_size_cur):
                X_adv_img = X_adv_cpu[img_i].permute(1, 2, 0)
                save_path = 'advimgs_un/ifgsm_UNIWARD/' + image_id_list[k * batch_size + img_i] + '.png'
                imageio.imwrite(save_path, X_adv_img)
...
for k in range(0, num_batches):
    ...
    for t in range(max_iterations):
        ...
        delta.data = delta.data + lr * cam_grad                         # un-targeted
        delta.data = delta.data.clamp(-epsilon / 255, epsilon / 255)
        delta.data = ((X_ori + delta.data).clamp(0, 1)) - X_ori
        if t % 50 == 49:
            X_adv = X_ori + delta
            ...
            for img_i in range(batch_size_cur):
                X_adv_img = X_adv_cpu[img_i].permute(1, 2, 0)
                save_path = 'advimgs_un/ifgsm_CAM/' + image_id_list[k * batch_size + img_i] + '.png'
                imageio.imwrite(save_path, X_adv_img)
...
for k in range(0, num_batches):
    ...
    for i in range(batch_size_cur):
        ...
        if rho_cam.max() == 0:
            print('all-zero cam in textural area')  # Never occur in our experiments
        else:
    ...
    for t in range(max_iterations):
        ...
        delta.grad.zero_()
        # delta.data = delta.data - lr * torch.sign(grad_c)
        rho_cam_grad = torch.sign(grad_c) * rho_cam_ts      # sign(delta)*NXiA
        ...
        if t % 50 == 49:
            ...
            for img_i in range(batch_size_cur):
                X_adv_img = X_adv_cpu[img_i].permute(1, 2, 0)
                save_path = 'advimgs_un/ifgsm_UNIWARD_CAM/' + image_id_list[k * batch_size + img_i] + '.png'
                imageio.imwrite(save_path, X_adv_img)
